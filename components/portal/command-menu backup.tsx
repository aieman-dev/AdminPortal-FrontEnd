...
export function CommandMenu() {
  ...
  const SEARCH_PATTERNS = [
    ...
    },
    {
        ...
        label: (q: string) => `Search Season Pass Plate: "${q.toUpperCase()}"`,
  ...
  const getDynamicActions = (): CommandItem[] => {
      ...
      const isReport = lowerQ.startsWith("rep") || lowerQ.startsWith("rep")
      const isPackage = lowerQ.startsWith("crea") || lowerQ.startsWith("pack")
      const isRegister = lowerQ.startsWith("New") || lowerQ.startsWith("reg")
      ...
      if (permissions.ITSupport && (isInvoice || isTicket)) {
          actions.push({ id: `void-${rawQ}`, label: `Void Transaction: "${rawQ}"`, icon: XCircle, path: `/portal/themepark-support/transaction-master?tab=void-transaction&search=${encodeURIComponent(rawQ)}`, priority: 1 });
          actions.push({ id: `deact-${rawQ}`, label: `Deactivate Ticket: "${rawQ}"`, icon: Ticket, path: `/portal/themepark-support/ticket-master?tab=deactivate-ticket&search=${encodeURIComponent(rawQ)}`, priority: 1 });
      ...
      if (isNumericID) {
        if (permissions.ITSupport) {
            actions.push({ id: `resync-${rawQ}`, label: `Resync Transaction ID: ${rawQ}`, icon: RefreshCw, path: `/portal/themepark-support/transaction-master?tab=resync-transaction&search=${encodeURIComponent(rawQ)}`, priority: 1 });
      ...
      // SCENARIO C: PEOPLE (Email, Phone, Name, report, package and register)
      if (isEmail || isPhone || isName || isReport || isPackage || isRegister) {
        if (permissions.ITSupport) {
            actions.push({ id: `find-acct-${rawQ}`, label: `Find Customer Account: "${rawQ}"`, icon: Users, path: `/portal/themepark-support/account-master?tab=account-management&search=${encodeURIComponent(rawQ)}`, priority: 1});
        }
          if (isEmail || isName) {
            if (permissions.MIS) {
                actions.push({ id: `find-staff-${rawQ}`, label: `Find Staff Member: "${rawQ}"`, icon: Briefcase, path: `/portal/staff-management?search=${encodeURIComponent(rawQ)}`, priority: 2});
            }
            if (permissions.carPark) {
                actions.push({ id: `email-cp-${rawQ}`, label: `Season Pass Search: ${rawQ}`, icon: Car, path: `/portal/car-park/season-parking?search=${encodeURIComponent(rawQ)}`, priority: 1 });
                actions.push({ id: `email-visitor-${rawQ}`, label: `Visitor Account Search: ${rawQ}`, icon: Users, path: `/portal/car-park/superapp-visitor?search=${encodeURIComponent(rawQ)}`, priority: 2 });
          ...
          if (isName) {
            ...
            if(permissions.packages){
                actions.push({ id: "nav-list-pkg", label: `Search Packages: "${rawQ}"`, icon: Ticket, path: `/portal/packages?search=${encodeURIComponent(rawQ)}`, priority: 2 });
            }
          }
          if (isReport || isRegister){
            if (permissions.carPark){
                actions.push({id: "nav-reports-priority",label: "Open Reports Center",icon: FileText,path: "/portal/car-park/reports",priority: 1});
                actions.push({id: "nav-new-register",label: "New Register",icon: UserPlus,path: "/portal/car-park/registration",priority: 1});
            }
          }
          if (isPackage){
            if(permissions.packages){
                actions.push({id: "create-package",label: "Create Package",icon: Ticket,path: "/portal/packages/form",priority: 1});
            }
          }
      }
