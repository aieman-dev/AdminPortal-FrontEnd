...
export default function RetailManualConsumeTab() {
  ...
  return (
    <>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 items-end">
                ...
                <div className="space-y-2">
                     ...
                     <Select value={tGroupId} onValueChange={setTGroupId}>
                         <SelectTrigger id="tGroupId"><SelectValue placeholder="Select Group" /></SelectTrigger>
                         <SelectContent>
      ...
      {consumeSearchResult && currentStep === 'selection' && (
        <>
              ...
              <div className="bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-t p-4 sticky bottom-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] rounded-b-lg">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-6 w-full md:w-auto">
                        ...
                        <Button onClick={handleNextStep} disabled={cartTotal === 0} size="lg" className="min-w-[150px]">
                            Next
                        </Button>
                    </div>
                </div>
      ...
      {consumeSearchResult && currentStep === 'confirmation' && (
            ...
            <Card>
                ...
                <CardContent className="space-y-6">
                    <div className="flex justify-between items-center bg-muted/50 p-4 rounded-lg border">
                        ...
                        <span className="text-xl font-bold text-primary">RM {consumeSearchResult.creditBalance.toFixed(2)}</span>
                    </div>
                    <Separator />
                    <div className="space-y-4">
                        <h4 className="text-sm font-medium text-muted-foreground">Order Summary:</h4>
                        {consumeSearchResult.items.filter(item => (quantities[item.id] || 0) > 0).map(item => {
                            const qty = quantities[item.id];
                            return (
