...
import { History, Calendar as CalendarIcon, Loader2, SearchX, LogOut, Search, Play, RotateCcw} from "lucide-react"
...
import { StatusBadge } from "@/components/themepark-support/it-poswf/status-badge" // Centralized Badge
...
export function ParkingActivityHistory({ accId }: ParkingActivityHistoryProps) {
   ...
   useEffect(() => {
    ...
    const handleForceExit = (row: ParkingActivity) => {
        toast.success("Action Triggered", `Force exit initiated for plate ${row.plateNo}`);
    }
    ...
    const columns: TableColumn<ParkingActivity>[] = [
        ...
        { 
            ...
            accessor: "plateNo", 
            cell: (val) => (
                // UPDATED: Normal font weight, simple badge style
        ...
        <Card className="w-full shadow-sm border border-gray-200 dark:border-zinc-800 animate-in fade-in slide-in-from-bottom-4">
            ...
            <CardContent className="p-0">
                <DataTable 
                    columns={columns}
                    data={history}
                    keyExtractor={(row, i) => `${row.entryTime}-${i}`}
                    isLoading={loading}
                    emptyIcon={SearchX}
