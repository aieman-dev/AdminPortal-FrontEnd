...
import { Calendar, User, Copy, Pencil, Trash2, Globe, Tag } from "lucide-react"; 
...
export default function PackageCard({
  ...
  // DISPLAY LOGIC: 
  // Entry/Reward P -> RM (Price column)
  // Point -> Pts (Price AND Point column, but Pts usually dominates display context)
  ...
  const displayPrice = numericPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

  return (
    <div
      onClick={onClick}
      className={`group relative bg-white dark:bg-gray-700 border cursor-pointer rounded-xl overflow-hidden transform transition-all duration-300 hover:shadow-lg
         ${isSelected ? 'border-indigo-500 ring-2 ring-indigo-500' : 'border-gray-200 dark:border-gray-700 hover:scale-[1.03]'}
      `}
    >
      {/* Image */}
      <div className="relative h-32">
        ...
        {isSelectable && (
            ...
            >
                <Checkbox 
                    ...
                    className="data-[state=checked]:bg-indigo-600 data-[state=checked]:border-indigo-600 border-white bg-white/80 w-5 h-5"
                />
            </div>
        )}
        ...
        {/* NEW: Package Type Badge (Right side of Bundle badge if space permits, or simply stacked/aligned) */}
        {/* For layout cleanliness, I'll put it top-right if not hovering, or adjust top-left stack */}
        <div className="absolute bottom-2 left-2 flex gap-1">
             <span className="bg-black/60 text-white text-[10px] px-2 py-0.5 rounded backdrop-blur-sm flex items-center gap-1">
                <Tag size={10} /> {packageType}
             </span>
        </div>
      ...
      <div className="p-3">
        ...
        <p className="text-sm text-blue-600 font-bold mb-2 dark:text-blue-400">
          ...
          {isPoint ? `${displayPrice} ${priceUnit}` : `${priceUnit} ${displayPrice}`}
        </p>
