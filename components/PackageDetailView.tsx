...
const DetailSkeleton = () => (
  // Matches the main container layout exactly
...
export default function PackageDetailView({ id, source }: PackageDetailViewProps) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)] w-full max-w-[1600px] mx-auto animate-in fade-in slide-in-from-bottom-2 duration-500 overflow-hidden bg-background">
      
      {/* 1. HEADER */}
      <div className="flex-shrink-0 flex items-center justify-between border-b px-4 py-3 bg-background/95 backdrop-blur z-10">
          <div className="flex items-center gap-4">
             ...
             <div>
                 <div className="flex items-center gap-3">
                    <h1 className="text-lg font-bold tracking-tight text-foreground">{packageData.name}</h1>
                    <StatusBadge status={packageData.status} className="px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider" />
                 </div>
                 <div className="flex items-center gap-3 text-xs text-muted-foreground font-mono mt-0.5">
                    <span>ID: {packageData.id}</span>
                    <span className="text-muted-foreground/30">•</span>
                    <span>{getPackageTypeLabel(packageData.packageType)}</span>
                 </div>
             </div>
          </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-full">
            ...
            <div className="lg:col-span-3 h-full flex flex-col min-h-0">
                <div className="flex flex-col h-full bg-card rounded-xl shadow-sm border ring-1 ring-black/5 dark:ring-white/10 overflow-hidden">
                        ...
                        {/* IMAGE */}
                        <div className="relative h-36 w-full bg-muted shrink-0 group overflow-hidden">
                            <img 
                                src={displayImage} 
                                alt="Package" 
                        ...
                        <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3 pb-6 scrollbar-hide">
                            ...
                            {isActionable ? (
                                <div className="mt-auto space-y-2 pt-2">
                                    <Button className="w-full h-10 bg-indigo-600 hover:bg-indigo-700 shadow-sm text-white font-semibold text-sm" onClick={() => setIsApprovalModalOpen(true)}>
                                        <CheckCircle2 className="w-4 h-4 mr-2" /> Approve
                                    </Button>
                                    
                                    <div className="bg-red-50 dark:bg-red-900/10 p-2 rounded-lg space-y-2 border border-red-100 dark:border-red-900/30">
                                        ...
                                        <Textarea 
                                            placeholder="Reason..." 
                                            className="min-h-[60px] text-sm resize-none bg-white dark:bg-black/20 border-red-200 focus-visible:ring-red-500"
                                ...
                                <div className="mt-auto pt-4">
                                    <div className="bg-muted/30 p-3 rounded-lg text-center border border-dashed text-muted-foreground">
            ...
            <div className="lg:col-span-9 h-full flex flex-col min-h-0">
                <div className="flex flex-col h-full bg-card rounded-xl border shadow-sm ring-1 ring-black/5 dark:ring-white/10 overflow-hidden">
                    <Tabs defaultValue="overview" className="flex flex-col h-full w-full">
                       ...
                       <div className="flex-1 overflow-y-auto p-3 bg-muted/5 pb-6 scrollbar-hide">
                            
                            {/* TAB 1: OVERVIEW */}
                            <TabsContent value="overview" className="mt-0 flex flex-col gap-3">
                                
                                {/* Row 1: Validity & Audit */}
                                    ...
                                    <Card className="col-span-1 md:col-span-2 shadow-none border flex flex-col">
                                        <CardHeader className="py-2 px-3 border-b bg-muted/20">
                                            <CardTitle className="text-xs font-semibold flex items-center gap-2">
                                                <Clock className="w-3.5 h-3.5 text-indigo-500"/> Validity Period
                                            </CardTitle>
                                        </CardHeader>
                                        <CardContent className="p-3">
                                            <div className="grid grid-cols-4 gap-4">
                                                ...
                                                <div className="text-center md:text-left border-r last:border-0 pr-2">
                                                    ...
                                                    <div className="text-xs text-muted-foreground bg-muted/50 p-1.5 rounded mt-1 inline-block">
                                                        {getTimeDisplay(packageData.effectiveDate) || "12:00 am"}
                                                    </div>
                                                </div>
                                                
                                                {/* EXPIRES DATE */}
                                                <div className="text-center md:text-left border-r last:border-0 pr-2 pl-2">
                                                    <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider block mb-1">Expires</span>
                                                    <div className="text-sm font-semibold">{formatDate(packageData.lastValidDate)}</div>
                                                    <div className="text-xs text-muted-foreground bg-muted/50 p-1.5 rounded mt-1 inline-block">
                                                        {getTimeDisplay(packageData.lastValidDate) || "12:00 am"}
                                                    </div>
                                                </div>
                                                <div className="text-center md:text-left border-r last:border-0 pr-2 pl-2">
                                    ...
                                    <Card className="col-span-1 shadow-none border flex flex-col">
                                        ...
                                        <CardContent className="p-3 flex flex-col gap-2 justify-center flex-1">
                                            <div className="flex justify-between items-start text-xs">
                                                <span className="text-muted-foreground font-medium mt-0.5">Created</span>
                                            ...
                                            <div className="flex justify-between items-start text-xs">
                                                <span className="text-muted-foreground font-medium mt-0.5">Reviewed</span>
                                                <div className="text-right">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 min-h-[120px]">
                                    ...
                                    <Card className="shadow-none border border-blue-200/60 dark:border-blue-900/30 flex flex-col">
                                        <CardHeader className="py-2 px-3 border-b bg-blue-50/50 dark:bg-blue-900/10">
                                            <CardTitle className="text-xs font-bold flex items-center gap-2 text-blue-700 dark:text-blue-400 uppercase tracking-wide">
                                                <FileText className="w-3.5 h-3.5" /> Themepark Remark
                                            </CardTitle>
                                        </CardHeader>
                                        <CardContent className="p-3">
                                    ...
                                    <Card className="shadow-none border border-amber-200/60 dark:border-amber-900/30 flex flex-col">
                                        <CardHeader className="py-2 px-3 border-b bg-amber-50/50 dark:bg-amber-900/10">
                                            <CardTitle className="text-xs font-bold flex items-center gap-2 text-amber-700 dark:text-amber-400 uppercase tracking-wide">
                                                <ShieldCheck className="w-3.5 h-3.5" /> Finance remark
                                            </CardTitle>
                                        </CardHeader>
                                        <CardContent className="p-3">
                                            <p className={`text-sm leading-relaxed whitespace-pre-wrap font-medium ${packageData.remark2 ? "text-foreground" : "text-muted-foreground"}`}>
                                                {packageData.remark2 || "No remark yet."}
                                            </p>
                                        </CardContent>
                                    </Card>
                                </div>
                            ...
                            <TabsContent value="items" className="mt-0 flex flex-col">
                                <div className="flex gap-2 mb-3 shrink-0">
                                    <div className="relative flex-1">
                                        ...
                                        <Input 
                                            ...
                                            className="pl-8 h-8 bg-background border shadow-sm text-sm" 
                                            value={searchQuery}
                                            onChange={(e) => setSearchQuery(e.target.value)}
                                        />
                                    </div>
                                    <div className="flex bg-background rounded-lg border p-0.5 gap-0.5 h-8 items-center shadow-sm">
                                        <Button variant="ghost" size="icon" className={`h-6 w-7 rounded-md ${isDenseView ? 'bg-muted shadow-sm' : ''}`} onClick={() => setIsDenseView(true)} title="Grid">
                                            <LayoutGrid className="w-3.5 h-3.5" />
                                        </Button>
                                        <Button variant="ghost" size="icon" className={`h-6 w-7 rounded-md ${!isDenseView ? 'bg-muted shadow-sm' : ''}`} onClick={() => setIsDenseView(false)} title="List">
                                ...
                                {filteredItems.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center border-2 border-dashed rounded-xl bg-muted/10 h-32">
                                            ...
                                            return isDenseView ? (
                                                <div key={index} className="group flex flex-col p-2.5 rounded-lg border bg-background hover:border-indigo-500/50 hover:shadow-sm transition-all duration-200">
                                                    <div className="flex justify-between items-start gap-2 mb-1">
                                                        <Badge variant="outline" className="h-4 px-1.5 font-mono text-[10px] bg-muted/50 border-transparent group-hover:bg-indigo-50 group-hover:text-indigo-700">
                                                            x{item.entryQty || 1}
                                                        </Badge>
                                                        <span className="text-[10px] font-mono text-muted-foreground/50">#{item.attractionId}</span>
                                                    </div>
                                                    <div className="font-semibold text-sm leading-tight line-clamp-2 mb-2 min-h-[2.4em] group-hover:text-indigo-700 transition-colors">
                                                        {item.itemName}
                                                    </div>
                                                    <div className="mt-auto pt-2 border-t border-dashed border-border/60 flex justify-between items-center">
                                                        ...
                                                        <span className="font-bold text-sm">
                                                            {formatCurrency(itemVal, isPointPackage(packageData.packageType))}
                                                        </span>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div key={index} className="flex items-center justify-between p-2.5 rounded-lg border bg-background hover:border-indigo-200 hover:shadow-sm transition-all">
                                                    ...
                                                    <div className="text-right">
                                                        <div className="font-bold text-sm">{formatCurrency(itemVal, isPointPackage(packageData.packageType))}</div>
                                                    </div>
                                                </div>
                                            );
                                        })}
