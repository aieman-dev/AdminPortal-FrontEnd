...
export default function DashboardClient({ initialPendingPackages }: DashboardClientProps) {
  ...
  // --- 2. OPTIMIZATION: USE SERVER DATA DIRECTLY ---
  // No useEffect needed! The data is already here.
  // We use useState just in case you want to filter/modify it later on the client.
  const [pendingPackages] = useState<Package[]>(initialPendingPackages);
  
  // Since data comes from server, we are never "loading"
